...
(function() {
  var Canvas, Css3Support, DiagonalSlider, debug, log, _ACOS, _ANGLE_TEXT, _ASIN, _COS, _MAX, _MIN, _PERCENT_DISTRIBUTION_CLIP, _PERCENT_DISTRIBUTION_CONT, _RADIAN, _SIN, _SKEW_ANGLE;
  ...
  Css3Support = (function() {
    ...
    function Css3Support() {
      var el;
      el = this.testElement ? this.testElement : (this.testElement = document.createElement('p'));
  ...
  DiagonalSlider = (function() {
    ...
    DiagonalSlider.prototype.reset = function(callbackEach, callbackComplete, $filter, $skipTransition) {
      var currentIndex, onSlideTransitionEnds, self, slides, supportTransform, transitionCntSlide;
      if ($filter == null) {
        $filter = null;
      }
      ...
      onSlideTransitionEnds = function(current) {
        if (current == null) {
      ...
      slides.each(function(i, slideElement) {
        ...
        if (Math.floor(pcssl) === Math.floor(tx) || $skipTransition) {
          if (callbackEach) {
        ...
        if (supportTransform) {
          ...
          slide.addClass('animate').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(event) {
            transitionCntSlide--;
            onSlideTransitionEnds();
    ...
    DiagonalSlider.prototype.open = function(index) {
      ...
      this.reset(function(ii, transitionSlide) {
        ...
        if (slidesCountTransitioning === 0) {
          return self.scope.removeClass('closed').addClass('open');
        }
      });
      return this;
    };
  ...
  if (typeof this.jQuery !== null) {
    $.fn.diagonalSlider = function(options) {
      var args, isMethodCall, methodResult, selection;
      if (options == null) {
        options = {};
      }
      selection = $(this);
      ...
      if (isMethodCall) {
        methodResult = (function() {
          ...
          method = args.shift();
          methodReference = method !== null ? diagonalSlider[method] : false;
          if (methodReference) {
            try {
